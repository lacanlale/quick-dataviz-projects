...
```{r}
total_per_country <- func_counts(data, CountryFormatted, Name)
mapped_data <- world_map |>
  left_join(total_per_country, by = c("name_en" = "CountryFormatted"))

geo_athlete_chart <- mapped_data |>
...
```{r}
total_per_weight <- func_counts(data, c(IPFWeightClassKG, Age, Sex), Name)

male_totals_per_weight <- 
  total_per_weight |>
  filter(Sex == "M" & IPFWeightClassKG != "Unknown") 

male_heatmap <- male_totals_per_weight |>
  ggplot(aes(x=IPFWeightClassKG, y=Age, fill=log(unique_total))) +
  geom_raster()

female_totals_per_weight <- 
  total_per_weight |>
  filter(Sex == "F" & IPFWeightClassKG != "Unknown") 

female_heatmap <- female_totals_per_weight |>
  ggplot(aes(x=IPFWeightClassKG, y=Age, fill=log(unique_total))) +
  geom_tile()

heatmap_chart <- male_heatmap + female_heatmap
...
#| fig-height: 7
(
  (geo_athlete_chart + pl_growth_chart) /
  (donut_gender_chart + heatmap_chart)
)
```
...
deadlift_elites |>
  ggplot(aes(x=Year, y=Best3DeadliftKg, color=rank_group)) +
  geom_point()

dots_elites |>
  ggplot(aes(x=Year, y=Dots, fill=Sex, color=rank_group)) +
  geom_point()
