...
[x] *Poster One - Horizontal Format* : The international growth of powerlifting at micro- and macro- level scales

[x] *Poster Two - Vertical Format* : The growing skill gap of elite level powerlifters
...
deadlift_elites |>
  ...
  ggplot(aes(x=Dots, y=fct_rev(factor(Year)), fill=rank_group, color=rank_group)) +
  geom_density_ridges(alpha=0.75) + 
  facet_wrap(fct_rev(Sex) ~ IPFWeightClassKG, ncol=2, dir="v") +
  scale_x_continuous(name="IPF Standard Weight Class (kg)") +
  ...
  labs(title="Total Athletes per IPF Standardized Weight Class")
```
...
func_ridge_plot <- function(data, time_col, val_col, group_col, sex_col, facet_group, x_label, title) {
  return(
    ...
    |>
      ...
      facet_wrap(vars({{ sex_col }}, {{ facet_group }}), ncol=2, dir="v") +
...
func_ridge_plot(deadlift_elites, Year, Best3DeadliftKg, rank_group, fct_rev(Sex), IPFWeightClassKG, "Total (kg)", "Deadlift")
```
...
(
  ...
  func_ridge_plot(dots_elites, Year, Dots, rank_group, fct_rev(Sex), IPFWeightClassKG, "DOTS Score", "DOTS")
)
