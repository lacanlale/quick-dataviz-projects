...
mapped_data <- world_map |>
  left_join(total_per_country, by = c("name_long" = "CountryFormatted"))
...
```{r}
squat_elites |>
  ggplot(aes(x=Year, y=Best3SquatKg, color=rank_group)) +
  geom_point()
...
wilks_elites |>
  ggplot(aes(x=Year, y=Dots, color=rank_group)) +
  geom_point()
```
